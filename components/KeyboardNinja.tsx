...
const gameData = [
  { question: 'Memilih seluruh baris', answer: 'SHIFT + SPACE', options: ['CTRL + SPACE', 'ALT + SPACE', 'F2'] },
  { question: 'Memilih seluruh kolom', answer: 'CTRL + SPACE', options: ['SHIFT + SPACE', 'CTRL + A', 'F4'] },
  { question: 'Mengaktifkan/menonaktifkan filter', answer: 'CTRL + SHIFT + L', options: ['CTRL + L', 'ALT + F', 'CTRL + T'] },
  { question: 'Membuat tabel dari rentang data', answer: 'CTRL + T', options: ['CTRL + A', 'CTRL + SHIFT + T', 'ALT + T'] },
  { question: 'Menyisipkan tanggal hari ini', answer: 'CTRL + ;', options: ['CTRL + :', 'CTRL + D', 'TODAY()'] },
];

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const KeyboardNinja: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleAnswerClick = (option: string) => {
    ...
    if (option === currentQuestion.answer) {
      ...
      setFeedback('Luar biasa! Tanganmu secepat kilat!');
    } else {
  ...
  if (!currentQuestion) {
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center">
        ...
        <p className="text-slate-600 mt-2">Skor akhir Anda sebagai Keyboard Ninja:</p>
        <p className="text-6xl font-bold text-gray-600 my-4">{score}/{questions.length}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-2xl font-bold text-slate-800">Keyboard Ninja</h2>
      ...
      <div className="p-6 bg-slate-100 rounded-lg text-center mb-6">
        <p className="font-semibold text-lg text-slate-800">Shortcut keyboard untuk {currentQuestion.question} adalah...</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {currentQuestion.shuffledOptions.map(option => (
          <button
            ...
            disabled={isAnswered}
            className="p-4 rounded-lg border-2 border-slate-200 bg-white hover:border-gray-500 hover:bg-gray-50 transition-colors disabled:cursor-not-allowed font-semibold text-slate-700 text-left"
          >
            {option}
          </button>
        ))}
      ...
      {feedback && (
        <p className={`mt-4 text-center font-semibold ${feedback.includes('Luar biasa') ? 'text-green-600' : 'text-red-600'}`}>{feedback}</p>
      )}
    </div>
  );
};
