...
const gameData = [
  { question: 'Untuk menunjukkan tren naik turun dari data penjualan bulanan dalam satu sel, sparkline jenis apa yang digunakan?', answer: 'Line', options: ['Column', 'Win/Loss', 'Pie'] },
  { question: 'Untuk membandingkan nilai positif dan negatif (misalnya, laba rugi), sparkline apa yang paling cocok?', answer: 'Win/Loss', options: ['Line', 'Column', 'Area'] },
  { question: 'Sparkline jenis "Column" menampilkan data sebagai...', answer: 'Grafik batang mini', options: ['Grafik garis mini', 'Titik data', 'Indikator menang/kalah'] },
  { question: 'Sparkline berada di tab menu apa?', answer: 'Insert', options: ['Data', 'Formulas', 'View'] },
];

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const SparklineSpeedster: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleAnswerClick = (option: string) => {
    ...
    if (option === currentQuestion.answer) {
      ...
      setFeedback('Tepat! Visualisasi yang cerdas!');
    } else {
  ...
  if (!currentQuestion) {
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center">
        ...
        <p className="text-slate-600 mt-2">Skor akhir Anda dalam Sparkline Speedster:</p>
        <p className="text-6xl font-bold text-purple-600 my-4">{score}/{questions.length}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-2xl font-bold text-slate-800">Sparkline Speedster</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {currentQuestion.shuffledOptions.map(option => (
          <button
            ...
            disabled={isAnswered}
            className="p-4 rounded-lg border-2 border-slate-200 bg-white hover:border-purple-500 hover:bg-purple-50 transition-colors disabled:cursor-not-allowed font-semibold text-slate-700 text-left"
          >
            {option}
          </button>
        ))}
