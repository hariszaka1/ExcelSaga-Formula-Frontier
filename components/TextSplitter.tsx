...
const gameData = [
  { question: 'Dari teks "ID-Produk-001", fungsi apa yang digunakan untuk mengambil "ID"?', answer: 'LEFT', options: ['RIGHT', 'MID', 'FIND'] },
  { question: 'Untuk mengambil "Produk" dari "ID-Produk-001", fungsi apa yang paling cocok?', answer: 'MID', options: ['LEFT', 'RIGHT', 'SEARCH'] },
  { question: 'Formula apa yang akan mengambil 3 karakter terakhir ("001") dari "ID-Produk-001" di sel A1?', answer: '=RIGHT(A1,3)', options: ['=LEFT(A1,3)', '=MID(A1,10,3)', '=TRIM(A1)'] },
  { question: 'Fungsi modern apa yang dapat memisahkan "ID-Produk-001" menjadi 3 sel berbeda menggunakan "-" sebagai pemisah?', answer: 'TEXTSPLIT', options: ['CONCAT', 'SUBSTITUTE', 'TEXTJOIN'] },
];

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const TextSplitter: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleAnswerClick = (option: string) => {
    ...
    if (option === currentQuestion.answer) {
      ...
      setFeedback('Tepat! Teks berhasil diurai!');
    } else {
  ...
  if (!currentQuestion) {
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center">
        ...
        <p className="text-slate-600 mt-2">Skor akhir Anda dalam Text Splitter:</p>
        <p className="text-6xl font-bold text-blue-600 my-4">{score}/{questions.length}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-2xl font-bold text-slate-800">Text Splitter</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {currentQuestion.shuffledOptions.map(option => (
          <button
            ...
            disabled={isAnswered}
            className="p-4 rounded-lg border-2 border-slate-200 bg-white hover:border-blue-500 hover:bg-blue-50 transition-colors disabled:cursor-not-allowed font-semibold text-slate-700 text-left"
          >
            {option}
          </button>
        ))}
