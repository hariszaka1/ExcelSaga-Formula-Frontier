...
const gameData = [
  { element: 'Judul Grafik (Chart Title)', description: 'Teks utama yang menjelaskan isi grafik.', options: ['Legenda', 'Sumbu Y', 'Garis Grid'] },
  { element: 'Legenda (Legend)', description: 'Penjelasan seri data yang berbeda dalam grafik.', options: ['Label Data', 'Judul Sumbu', 'Area Plot'] },
  { element: 'Sumbu Y (Vertical Axis)', description: 'Sumbu vertikal yang biasanya menunjukkan nilai.', options: ['Sumbu X', 'Garis Tren', 'Judul Grafik'] },
  { element: 'Label Data (Data Labels)', description: 'Label yang menunjukkan nilai pasti untuk setiap titik data.', options: ['Legenda', 'Sumbu X', 'Garis Grid'] },
  { element: 'Garis Grid (Gridlines)', description: 'Garis horizontal atau vertikal di latar belakang untuk membantu membaca nilai.', answer: 'Garis Grid', options: ['Garis Tren', 'Area Plot', 'Sumbu Y'] },
];

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const ChartElementId: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleAnswerClick = (option: string) => {
    ...
    if (option === currentQuestion.description) {
      ...
      setFeedback('Tepat! Anda seorang ahli grafik!');
    } else {
  ...
  if (!currentQuestion) {
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center">
        ...
        <p className="text-slate-600 mt-2">Skor akhir Anda dalam Chart Element ID:</p>
        <p className="text-6xl font-bold text-amber-600 my-4">{score}/{questions.length}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-2xl font-bold text-slate-800">Chart Element ID</h2>
      ...
      <div className="p-6 bg-slate-100 rounded-lg text-center mb-6">
        <p className="font-semibold text-slate-700">Apa fungsi dari elemen grafik berikut?</p>
        <p className="text-2xl font-bold text-slate-800 mt-2">{currentQuestion.element}</p>
      </div>
      <div className="grid grid-cols-1 gap-3">
        {currentQuestion.shuffledOptions.map(option => (
          <button
            ...
            disabled={isAnswered}
            className="p-4 rounded-lg border-2 border-slate-200 bg-white hover:border-amber-500 hover:bg-amber-50 transition-colors disabled:cursor-not-allowed font-semibold text-slate-700 text-left"
          >
            {option}
          </button>
        ))}
