...
const gameData = [
  { question: 'Untuk membekukan hanya baris pertama (Row 1), opsi apa yang Anda pilih?', answer: 'Freeze Top Row', options: ['Freeze First Column', 'Freeze Panes', 'Unfreeze Panes'] },
  { question: 'Untuk melihat baris header (Row 1) dan kolom ID Produk (Column A) saat scroll, di sel manakah Anda harus meletakkan kursor sebelum memilih "Freeze Panes"?', answer: 'B2', options: ['A1', 'B1', 'A2'] },
  { question: 'Untuk membekukan hanya kolom pertama (Column A), opsi apa yang Anda pilih?', answer: 'Freeze First Column', options: ['Freeze Top Row', 'Freeze Panes', 'Split'] },
  { question: 'Apa fungsi dari "Unfreeze Panes"?', answer: 'Menghapus semua pembekuan di worksheet', options: ['Membekukan baris dan kolom saat ini', 'Menyembunyikan panel', 'Membuat panel baru'] },
];

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const FreezePanesPuzzle: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleAnswerClick = (option: string) => {
    ...
    if (option === currentQuestion.answer) {
      ...
      setFeedback('Tepat! Navigasi Anda kini lebih mudah!');
    } else {
  ...
  if (!currentQuestion) {
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center">
        ...
        <p className="text-slate-600 mt-2">Skor akhir Anda dalam Freeze Panes Puzzle:</p>
        <p className="text-6xl font-bold text-fuchsia-600 my-4">{score}/{questions.length}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-2xl font-bold text-slate-800">Freeze Panes Puzzle</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {currentQuestion.shuffledOptions.map(option => (
          <button
            ...
            disabled={isAnswered}
            className="p-4 rounded-lg border-2 border-slate-200 bg-white hover:border-fuchsia-500 hover:bg-fuchsia-50 transition-colors disabled:cursor-not-allowed font-semibold text-slate-700 text-left"
          >
            {option}
          </button>
        ))}
