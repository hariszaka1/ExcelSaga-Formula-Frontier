...
const gameData = [
  { question: 'Untuk mengurutkan data dari nilai terbesar ke terkecil, Anda memilih urutan...', answer: 'Descending (Z to A)', options: ['Ascending (A to Z)', 'Custom List', 'By Color'] },
  { question: 'Fitur "Filter" di Excel biasanya diaktifkan dengan shortcut apa?', answer: 'CTRL + SHIFT + L', options: ['CTRL + F', 'CTRL + T', 'ALT + ='] },
  { question: 'Untuk menampilkan hanya baris yang berisi "Jakarta" di kolom "Kota", filter apa yang Anda gunakan?', answer: 'Text Filter: Equals "Jakarta"', options: ['Number Filter: Greater Than', 'Date Filter: This Week', 'Advanced Filter'] },
  { question: 'Apa fungsi dari "Clear Filter"?', answer: 'Menghapus filter dari satu kolom', options: ['Menghapus semua filter di sheet', 'Menghapus data yang difilter', 'Menyimpan filter saat ini'] },
];

const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);

export const SortFilterSprint: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleAnswerClick = (option: string) => {
    ...
    if (option === currentQuestion.answer) {
      ...
      setFeedback('Benar! Data Anda kini lebih teratur!');
    } else {
  ...
  if (!currentQuestion) {
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center">
        ...
        <p className="text-slate-600 mt-2">Skor akhir Anda dalam Sort & Filter Sprint:</p>
        <p className="text-6xl font-bold text-pink-600 my-4">{score}/{questions.length}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
      <div className="flex justify-between items-center mb-1">
        <h2 className="text-2xl font-bold text-slate-800">Sort & Filter Sprint</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {currentQuestion.shuffledOptions.map(option => (
          <button
            ...
            disabled={isAnswered}
            className="p-4 rounded-lg border-2 border-slate-200 bg-white hover:border-pink-500 hover:bg-pink-50 transition-colors disabled:cursor-not-allowed font-semibold text-slate-700 text-left"
          >
            {option}
          </button>
        ))}
