...
import type { LevelCompletionStatus, UserView, User, CertificateRecord, CertificateSettings, PasswordPolicy } from './types';
...
export const api = {
  ...
  adminCreateUser: (details: { fullName: string; phone: string; email: string; password_input: string; isMember: boolean; }): Promise<{ user: User | null; error?: string }> => {
    return new Promise((resolve) => {
        setTimeout(() => {
            const { fullName, phone, email, password_input, isMember } = details;
            const lowerCaseEmail = email.toLowerCase();
            if (db.users[lowerCaseEmail]) {
                resolve({ user: null, error: 'Email sudah terdaftar.' });
                return;
            ...
            if (password_input.length < 6) {
                 resolve({ user: null, error: 'Password harus minimal 6 karakter.' });
                return;
            }
  ...
  getPasswordPolicy: (): Promise<{ policy: PasswordPolicy }> => {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve({ policy: db.passwordPolicy });
        }, NETWORK_DELAY / 2);
    });
  },

  updatePasswordPolicy: (newPolicy: PasswordPolicy): Promise<{ success: boolean, error?: string }> => {
    return new Promise((resolve) => {
        setTimeout(() => {
            db.passwordPolicy = newPolicy;
            saveDb();
            resolve({ success: true });
        }, NETWORK_DELAY);
    });
